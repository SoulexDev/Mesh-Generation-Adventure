    ...
    public void GenerateStructure(Vector3 pos, Transform terrainChunk, int biomeID, int spawnableID)
    {
        if (world != null)
        {
            for (int x = 0; x <= world.terrainSettings.chunkWidth; x++)
            {
                ...
                {
                    float spawnableLocation = world.noiseSettings.GetTerrainGenerationFromNoise(new Vector3(x + pos.x, 0, z + pos.z), world.terrainSettings.chunkWidth, 1, biomeID);
                    float spawnablePool = world.noiseSettings.GetStructureMask(new Vector3(x + pos.x, 0, z + pos.z), world.terrainSettings.chunkWidth, 1, biomeID, spawnableID);
